...
internal class CSharpFileBuilder : ISourceBuilder
{
    ...
    {
        var classBuilder = new ClassBuilder(isStatic, "B" + name, inheritances.Select(i => "B" + i).ToArray());
    ...
    public string Generate(int indent = 0)
    {
        ...
        sb.AppendLineWithIndent($"// Autogenerated by the NanosSharp Server Bindings Generator (c) {DateTime.Now.Year} DasDarki / GPLv3", indent).AppendLine();
